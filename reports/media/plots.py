...
TEST_NAMES = ["SW/ECB", "SW/CBC", "MMIO/ECB (automatic)",
              "MMIO/ECB (interactive)", "MMIO/CBC", "RoCC/ECB",
              "RoCC/CBC", "Hybrid/EBC"]
...
if __name__ == "__main__":
    ...
    fig, ax = plt.subplots()
    ecb_benchmarks = ["2-way\nBOOM", "MMIO\n(auto)", "MMIO\n(single)",
                      "RoCC", "Hybrid\n(batch)"]
    ecb_cycles = [boom_cycles[0], rocket_cycles[2],
                  rocket_cycles[3], rocket_cycles[5], rocket_cycles[7]]
    ...
    ax.barh(y_pos, ecb_sorted_speedup, align="center", color="steelblue", height=0.5, edgecolor="black")
    ax.set_yticks(y_pos)
    ax.set_xticks(numpy.arange(0, 5, 0.5))
    ax.set_yticklabels(ecb_sorted_names)
    ax.set_xlabel("Speedup")
    ax.set_title("Speedup compared to pure software in-order implementation\n(ECB mode)")
    ...
    cbc_cycles = [boom_cycles[1], rocket_cycles[4],
                  rocket_cycles[6]]
    ...
    ax.barh(y_pos, cbc_sorted_speedup, align="center", color="steelblue", height=0.5, edgecolor="black")
    ax.set_yticks(y_pos)
