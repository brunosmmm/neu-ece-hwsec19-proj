...
\begin{tikzpicture}
  ...
  \begin{scope}[name=completeacc, local bounding box=accbb, shift={(3, 0.2)}]
    \node (reg0) [accint, minimum width=25pt, xshift=-5pt, minimum height=5pt, font=\tiny] {REG 0};
    \node (reg1) [accint, minimum width=25pt, minimum height=5pt, below=of reg0, font=\tiny] {REG 1};
    \node (reg2) [accint, minimum width=25pt, minimum height=5pt, below=of reg1, font=\tiny] {REG N};
  ...
  \draw[-latex] (hwacc1.west) -- ++(-1,0) node [anchor=south west] {\tiny int};
  \draw[-latex] (hwacc2.west) -- ++(-0.25, 0) -- ++(0, 0.9) -- ++(-2.75, 0) node [anchor=south west] {\tiny int};
  \end{scope}
  ...
  \node [fit=(socbb), draw, dashed, rounded corners, thick, inner sep=6pt, label={320:SoC}] {};
